...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/TabbyCat/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/TabbyCat/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/TabbyCat/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/TabbyCat/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/TabbyCat/models.json", base), fmt.Sprintf("%s/api/TabbyCat/vendors.json", base)
}
