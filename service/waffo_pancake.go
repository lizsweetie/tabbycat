...
// are exposed separately for self-service flows driven from tabbycat's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("tabbycat-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "tabbycat-store"
	defaultWaffoPancakeProductName = "tabbycat-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because tabbycat has no renewal-
// event handling; Pancake auto-renewing without tabbycat extending user
